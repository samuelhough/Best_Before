...
var BestDate = require('../lib/Best_Before.js'),
...
exports['BestDate'] = {
  ...
  'All real leap years pass ': function(test) {
    ...
    for(var cur = 0; cur < leapYears.length; cur++){
      ...
      test.equal(leapYear.result, leapYears[cur]+'-02-29');
    }
    test.done();
  },
  'Invalid dates will fail': function(test){
      ...
      badDates.forEach( function( date ){
        var leapYear = new BestDate( date );
        test.equal( date+' is illegal', leapYear.result )
      });
      test.done();
  },
  'Out of bound dates will break': function(test){
...
// Check month lengths are valid
    ...
    months.forEach( function( oob ){
      var leapYear = new BestDate( oob );
      test.equal( oob+' is illegal', leapYear.result )
    })
    test.done();
  },
  'Legal days will produce valid formatted results': function( test ){
    ...
    legalDates.forEach( function( date, i ){
      var legal = new BestDate( date );
      test.equal( legal.result, formattedResult[i] );
    });
    test.done();
  }
}
